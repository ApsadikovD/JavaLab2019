                ...
                while (!isFinished) {
                    try {
                        ...
                        if (command.equals("login")) {
                            ...
                            Response<List<Message>> messages = mapper.readValue(line,
                                    new TypeReference<Response<List<Message>>>() {
                                    });
                            messages.getData().stream()
                                    .forEach(message -> System.out.println(message.getUsername()
                                            + ": " + message.getMessage()));
                        } else if (command.equals("product/get")) {
                            Response<List<Product>> products = mapper.readValue(line,
